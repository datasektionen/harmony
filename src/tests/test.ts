 ...
 */
async function readSubcommandMappings(
	subCommandConfig: SubCommandConfiguration[]
): Promise<
	Map<
		string,
		(interaction: GuildChatInputCommandInteraction) => Promise<void>
...
> {
	...
	for (const conf of subCommandConfig) {
		if (
			!conf.subCommand ||
			!conf.subCommandFilePath ||
			!conf.subCommandHandlerFilePath ||
			!conf.subCommandHandlerName
		) {
			log.error(
				`Invalid subcommand configuration: ${JSON.stringify(conf)}`
			);
			continue;
		}
...
export async function initTestCommand(): Promise<number> {
	...
	if (subCommandConfig !== null) {
		subcommandHandlerMapping = await readSubcommandMappings(
			subCommandConfig
		);
