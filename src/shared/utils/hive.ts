export async function getHiveGroups(tagId: string): Promise<HiveTagGroups> {
	const res = await fetch(`https://hive.datasektionen.se/api/v1/tagged/${tagId}/groups`, {
		method: "get",
		headers: {
...
export async function getHiveGroupMembers(groupId: string, groupDomain: string): Promise<string[]> {
	const res = await fetch(`https://hive.datasektionen.se/api/v1/group/${groupDomain}/${groupId}/members`, {
		method: "get",
		headers: {
			"Content-Type": "application/json",
            "Authorization": `Bearer ${process.env.HIVE}`
	...
	if (res.status !== 200) {
		throw new Error(`Error fetching members of group ${groupId} from domain ${groupDomain}.`);
	}
