...
import { createTestUpdateDfunktRolesJob} from '../tests/dfunkt-roles-update/tests';
import { Client as DiscordClient} from 'discord.js';
import { updateDiscordDfunktRoles } from './update-dfunk-roles-get-post';


export function initJobs(client: DiscordClient) : Map<string, {client: DiscordClient, job: CronJob<any, any>}> {
   // List of all CronJobs to start.
    let jobs : Map<string, {client: DiscordClient, job: CronJob<any, any>}> = new Map();
    
    // const testUpdateDfunktRolesJob: CronJob<any, any> = createTestUpdateDfunktRolesJob(client); // For testing purposes only
    const updateDfunktRolesJob: CronJob<any, any> = createUpdateDfunktRolesJob(client);
    
    // jobs.set('testUpdateDfunktRoles', {client: client, job: testUpdateDfunktRolesJob}); // For testing purposes only
    jobs.set('updateDfunktRoles', {client: client, job: updateDfunktRolesJob});
    return jobs;
}
 ...
 * */ 
const createUpdateDfunktRolesJob = (client: DiscordClient) => {
    let retryCount = 0;
    let job: CronJob;
    ...
    const onTick = async () => {
      try {
        ...
        if (retryCount < 5) {
            ...
            console.log('Max retries reached. Resetting to original interval.');
            retryCount = 0;
            await sendWebHookError("Harmony Error: Error during Discord dfunkt role update. Please take manual action by using the '/dfunkt' command on Discord.").then(res => {
  ...
  async function sendWebHookError(message: string) : Promise<Response> {
    ...
    const response = await fetch('https://mattermost.datasektionen.se/hooks/g6zrwz6r6p8fbjmz8ob1d7iaxw', {
        method: "POST",
        headers: headers,
        body: JSON.stringify({
            text: message,
            channel: "uptimerobot",
            username: "harmony",
            icon_url: "https://dsekt-assets.s3.amazonaws.com/shield-color-white-delta.png",
        }),
    });
    return response;
}
