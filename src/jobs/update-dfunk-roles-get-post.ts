...
import { Roles as DfunktRoles, Role as DfunktRole, Mandate as DfunktMandate, Roles} from "../shared/utils/dfunkt-interfaces";
import { getAllRoles } from "../shared/utils/dfunkt";
import {dfunktGroupToDiscordRoleMapping, dfunktToDiscordRoleMappings, discordDfunktRole, discordDfunktRolesIds } from "./dfunkt-roles-mapping";
import {testDfunktGroupToDiscordRoleMapping, testDfunktToDiscordRoleMappings, testDiscordDfunktRole, testDiscordDfunktRolesIds } from "../tests/dfunkt-roles-update/test-dfunkt-roles-mapping";
...
export async function updateDiscordDfunktRoles(
    ...
    // Process
    const processedDiscordData = processDiscordData(processedDfunktdata, dbUsers, discordData, testing);
    // POST
...
function processDfunktData(dfunktData : DfunktRoles) {
    ...
    let currentMandates : Map<string, DfunktRole[]> = new Map();
    ...
    let currentGroups : Map<string, string[]> = new Map();
    ...
    let hasHadMandates : Set<string> = new Set();
    let dfunktDiscordRoleLegible : Set<string> = new Set();
    ...
    dfunktData.forEach((role: DfunktRole) => {
        ...
        role.Mandates!.forEach((mandate: DfunktMandate) => {
            ...
            if (new Date(mandate.start) <= new Date() && new Date() <= new Date(mandate.end)) {
    ...
    hasHadMandates.forEach((kthid: string) => {
        if (!currentMandates.has(kthid))
            dfunktDiscordRoleLegible.add(kthid);        
    })
...
function processDiscordData(
    ...
    let toRemoveFromRole: Map<string, string[]> = new Map();
    let toAddToRole: Map<string, string[]> = new Map();
    ...
    if (testing) {
        dfunktProcessedData.currentMandates.forEach((roles: DfunktRole[], kthid: string) => {
            roles.forEach((role: DfunktRole) => {
                dfunktKthDiscordUsers.has(kthid)
                ? toAddToRole.has(dfunktKthDiscordUsers.get(kthid)!)
                  ? toAddToRole.get(dfunktKthDiscordUsers.get(kthid)!)!.push(testDfunktToDiscordRoleMappings.get(role.identifier)!)
                  : toAddToRole.set(dfunktKthDiscordUsers.get(kthid)!, [testDfunktToDiscordRoleMappings.get(role.identifier)!])
        ...
        dfunktProcessedData.currentMandates.forEach((roles: DfunktRole[], kthid: string) => {
            roles.forEach((role: DfunktRole) => {
                dfunktKthDiscordUsers.has(kthid)
                ? toAddToRole.has(dfunktKthDiscordUsers.get(kthid)!)
                  ? toAddToRole.get(dfunktKthDiscordUsers.get(kthid)!)!.push(dfunktToDiscordRoleMappings.get(role.identifier)!)
                  : toAddToRole.set(dfunktKthDiscordUsers.get(kthid)!, [dfunktToDiscordRoleMappings.get(role.identifier)!])
                : console.log("Error fetching user with KTH ID " + kthid); // This should never happen
            })
        })
    }
    ...
    // Group roles to add
    if (testing) {
        dfunktProcessedData.currentGroups.forEach((kthids: string[], groupIdentifier: string) => {
            kthids.forEach((kthid: string) => {
                dfunktKthDiscordUsers.has(kthid)
                ? toAddToRole.has(dfunktKthDiscordUsers.get(kthid)!)
                  ? toAddToRole.get(dfunktKthDiscordUsers.get(kthid)!)!.push(testDfunktGroupToDiscordRoleMapping.get(groupIdentifier)!)
                  : toAddToRole.set(dfunktKthDiscordUsers.get(kthid)!, [testDfunktGroupToDiscordRoleMapping.get(groupIdentifier)!])
                : console.log("Error fetching user with KTH ID " + kthid); // This should never happen
            })
        })
    }
        ...
        dfunktProcessedData.currentGroups.forEach((kthids: string[], groupIdentifier: string) => {
            kthids.forEach((kthid: string) => {
                dfunktKthDiscordUsers.has(kthid)
                ? toAddToRole.has(dfunktKthDiscordUsers.get(kthid)!)
                  ? toAddToRole.get(dfunktKthDiscordUsers.get(kthid)!)!.push(dfunktGroupToDiscordRoleMapping.get(groupIdentifier)!)
                  : toAddToRole.set(dfunktKthDiscordUsers.get(kthid)!, [dfunktGroupToDiscordRoleMapping.get(groupIdentifier)!])
                : console.log("Error fetching user with KTH ID " + kthid); // This should never happen
            })
        })
    }
    ...
    if (testing) {
        dfunktProcessedData.dfunktDiscordRoleLegible.forEach((kthid: string) => {
            dfunktKthDiscordUsers.has(kthid)
            ? toAddToRole.has(dfunktKthDiscordUsers.get(kthid)!)
                ? toAddToRole.get(dfunktKthDiscordUsers.get(kthid)!)!.push(testDiscordDfunktRole)
                : toAddToRole.set(dfunktKthDiscordUsers.get(kthid)!, [testDiscordDfunktRole])
            : console.log("Error fetching user with KTH ID " + kthid); // This should never happen
        })
    }
    else {
        dfunktProcessedData.dfunktDiscordRoleLegible.forEach((kthid: string) => {
            dfunktKthDiscordUsers.has(kthid)
            ? toAddToRole.has(dfunktKthDiscordUsers.get(kthid)!)
                ? toAddToRole.get(dfunktKthDiscordUsers.get(kthid)!)!.push(discordDfunktRole)
                : toAddToRole.set(dfunktKthDiscordUsers.get(kthid)!, [discordDfunktRole])
            : console.log("Error fetching user with KTH ID " + kthid); // This should never happen
        })
    }
    ...
    if (testing) {
        retainedRoles = discordData.guildRoles.filter((role, key) => testDiscordDfunktRolesIds.includes(key));
    }
    else {
        retainedRoles = discordData.guildRoles.filter((role, key) => discordDfunktRolesIds.includes(key));
    } 
 ...
 */
async function fetchFromDfunktAPI(){
...
async function fetchFromDb(
    processedDfunktData: ReturnType<typeof processDfunktData>
        ...
        let dfunktKthDiscordUsers: Map<string, string> = new Map();
        for (const [kthid, _] of processedDfunktData.currentMandates) {
            ...
            if (resolvedDiscordId != null)
                ...
                console.warn("Error fetching Discord ID for user with kthid " + kthid + ", they will be ignored during the update.");
            }
        }
 ...
 */
async function fetchFromDiscordAPI(guild: Guild) {
