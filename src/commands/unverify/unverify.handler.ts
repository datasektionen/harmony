...
export async function handleUnverify(
    ...
    if (!success) {
        interaction.editReply({ content: "Unverification failed under mysterious circumstances. Contact D-Sys to unverify the user." });
    ...
    Promise.all(guilds.map(async (e): Promise<void> => {
        const guild = await e.fetch();
        removeRole(user, "verified", guild);
    }));

    interaction.editReply({ content: "You have been unverified!" });
    log.info(`Successfully unverified user with user.id = "${user.id}", user.username = "${user.username})".`)
}