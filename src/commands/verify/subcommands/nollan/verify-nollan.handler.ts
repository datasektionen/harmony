...
export const handleVerifyNollan = async (
	...
	try {
		...
		Promise.all(courseCodes.map(async code => {
			try {
		...
		await interaction.editReply({ content: "You are now verified! You have been added to all course channels of your current year. \nYou can join or leave course channels with the `/join` and `/leave` command. \nFor more info, see: <#1020725853157593219>" });
	} catch (error) {
		console.warn(error);
		await interaction.reply({
